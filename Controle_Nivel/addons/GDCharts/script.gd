...
export(int, 6, 50) var MAX_VALUES = 12
...
func animation_move_dot(node, end_pos, end_scale, delay = 0.0, duration = 0.5):
	...
	if UseAnimation:
		...
		node.rect_position = end_pos
		node.rect_scale = end_scale
		_update_draw(1.0)
	...
	for index in range(0, ordinary_factor):
		...
		if abs(amplitude) > computed_ratio:
			...
			for index1 in range(-6, 6):
				ordinate_values.push_back(5 * index1 * computed_ratio / ordinary_factor)
